	...
	<div class="row">
		<div class="span12">
			<div class="well well-small">
				<form:form action="import${ id }/addorupdate" method="POST"
					...
					<div class="controls">
						...
						<form:select name="id" path="id" style="max-width: 170px">
							<c:forEach var="item" items="${ importDetailsPaginate }">
			...
			<table class="table table-bordered table-condensed">
				...
				<tbody>
					<c:if test="${ importDetailsPaginate.size() > 0 }">
						<c:forEach var="item" items="${ importDetailsPaginate }">
							<tr>
								...
								<td><a
									href="<c:url value="/quan-tri/nhap-hang/import${ id }/delete/${ item.id }"/>"
									class="btn btn-mini btn-danger" type="button"> <span
										class="icon-remove"></span>
								</a></td>
								<c:if
									test="${ (loop.index + 1) % 3 == 0 || (loop.index + 1)  == importDetailsPaginate.size() }">
							</tr>
							<c:if test="${ (loop.index + 1) < importDetailsPaginate.size() }">
							</c:if>
					</c:if>
					</c:forEach>
					</c:if>
	...
	<div class="pagination">
		<c:forEach var="item" begin="1" end="${ paginateInfo.totalPage }"
			...
			<c:if test="${ (loop.index) == paginateInfo.currentPage }">
				<a
					href="<c:url value="/quan-tri/nhap-hang/import${ id }/${ loop.index }"/>"
					class="active">${ loop.index }</a>
			</c:if>
			<c:if test="${ (loop.index) != paginateInfo.currentPage }">
				<a
					href="<c:url value="/quan-tri/nhap-hang/import${ id }/${ loop.index }"/>">${ loop.index }</a>
			</c:if>
		</c:forEach>
	</div>
</body>